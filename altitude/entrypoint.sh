...
for MAP in $(echo $MAPLIST | tr "," "\n"); do
    MAPLISTSTR="$MAPLISTSTR        \<String value=\"$MAP\" \/\>\\n"    
    echo $MAP
done

sed -i "s/\$MAPLIST/${MAPLISTSTR}/" $SERVER_DIR/$SERVER_SUB_DIR/$SERVER_CFG

cat $SERVER_DIR/$SERVER_SUB_DIR/$SERVER_CFG
