...
base = ["./srcds_run", "-game", "csgo", "-console", "-usercon", "+ip 0.0.0.0"]
...
if vars.get('SV_PASSWORD'):
    f = open('/csgo/csgo/cfg/server.cfg', 'a')
...
if vars.get('MAXPLAYERS_OVERRIDE'):
    base.append('-maxplayers_override {MAXPLAYERS_OVERRIDE}'.format(**vars))
    f = open('/csgo/csgo/cfg/server.cfg', 'a')
    f.write('\nmaxplayers "{MAXPLAYERS_OVERRIDE}"'.format(**vars))
    f.close()
...
if vars.get('TEAM1') and vars.get('TEAM2'):
    f = open('/csgo/csgo/cfg/names.cfg', 'w')
    f.write('mp_teamname_1 {TEAM1}\nmp_teamname_2 {TEAM2}'.format(**vars))
    f.close()
...
if vars.get('STEAMTOKEN'):
    ...
    f = open('/csgo/csgo/cfg/gamemode_competitive_server.cfg', 'a')
    f.write('\nsv_lan 1\n')
    f.close()
